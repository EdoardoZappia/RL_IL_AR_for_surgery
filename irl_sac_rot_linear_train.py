...
# Reward Network - modello lineare
class RewardNetwork(nn.Module):
    def __init__(self, state_dim=2, action_dim=1):
        super().__init__()
        input_dim = state_dim + action_dim  # 3 scalari: delta_before, action, delta_after
        self.linear = nn.Linear(input_dim, 1).to(device)

    def forward(self, delta_before, action, delta_after):
        x = torch.cat([delta_before, action, delta_after], dim=-1)  # [batch_size, 3]
...
if __name__ == "__main__":
    ...
    torch.save(reward_net.state_dict(), "IL/DME_SAC/reward_network_rot_0.5_0.01_linear.pt")

    agent.save("IL/SAC_POLICY/sac_with_learned_reward_rot_0.5_0.01_linear_IRL")
