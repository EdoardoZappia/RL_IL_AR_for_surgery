...
def train_bc_model(model, dataloader, num_epochs=200, learning_rate=1e-3,
                   device=None, save_path="IL/Loss_data/loss_curve.png"):
    ...
    loss_history = []  # lista per salvare le loss
    ...
    for epoch in range(num_epochs):
        ...
        loss_history.append(avg_loss)  # salva la loss per l’epoca
    ...
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.title("Behavioral Cloning Loss Curve")
    plt.legend()
    plt.grid(True)
    plt.savefig(save_path)  # salva come immagine
    plt.close()
