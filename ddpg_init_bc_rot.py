...
RUN_DIR = f"TEST_NOISE/Rotazioni-dinamiche/ddpg_mov_0.01_std_0.008_{now}"
...
def train_ddpg(env=None, num_episodes=10001):
    ...
    for episode in range(num_episodes):
        ...
        state = state.clone()
        state[1:] += torch.normal(mean=0.0, std=0.008, size=(1,), device=state.device)
        ...
        while not done:
            ...
            next_state[1:] += torch.normal(mean=0.0, std=0.008, size=(1,), device=next_state.device)
