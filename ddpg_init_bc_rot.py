...
#RUN_DIR = f"Esperimento_1_/Rotazioni-dinamiche/ddpg_mov_0.01_std_0.004_frozen_policy_{now}"
RUN_DIR = f"TEST_NOISE/Rotazioni-dinamiche/ddpg_mov_0.01_std_0.006_{now}"
...
def train_ddpg(env=None, num_episodes=10001):
    ...
    for episode in range(num_episodes):
        ...
        state = state.clone()
        state[1:] += torch.normal(mean=0.0, std=0.006, size=(1,), device=state.device)
        ...
        while not done:
            ...
            next_state[1:] += torch.normal(mean=0.0, std=0.006, size=(1,), device=next_state.device)
