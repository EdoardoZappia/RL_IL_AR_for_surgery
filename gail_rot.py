...
# === POLICY ===
class GaussianPolicy_rot(nn.Module):
...
def rollout(policy, env, max_steps=100):
    ...
    for _ in range(max_steps):
        obs_rot = torch.tensor([state[2], state[5]], dtype=torch.float32)
        dist = policy(obs_rot.unsqueeze(0))
        action = dist.sample().squeeze(0)
        log_prob = dist.log_prob(action).sum()

        # Costruisci azione completa per l'env
        full_action = torch.zeros(3)
        full_action[2] = action  # dtheta

        next_state, _, done, truncated, _, _ = env.step(full_action.detach().numpy())

        obs_list.append(obs_rot)
...
def train_gail(policy, discriminator, expert_data, num_iterations=1000, device="cpu"):
    ...
    env.close()
    torch.save(policy.state_dict(), "IL/gail_policy_rot.pth")
    print("\nGAIL training terminato e policy salvata in 'IL/gail_policy_rot.pth'")

# === AVVIO ===
if __name__ == "__main__":
    expert_data = np.load("trajectories/dataset_rot.npz")
    policy = GaussianPolicy_rot(obs_dim=2, act_dim=1)
    discriminator = Discriminator(obs_dim=2, act_dim=1)
