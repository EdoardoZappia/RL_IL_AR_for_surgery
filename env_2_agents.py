...
class TrackingEnv(gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        self.action_space_trasl = spaces.Box(low=np.array([-5, -5]), high=np.array([5, 5]), dtype=np.float32)
        self.action_space_rot = spaces.Box(low=np.array([-5]), high=np.array([5]), dtype=np.float32)
    ...
    def step(self, action):
        ...
        obs_trasl = np.concatenate([self.data.qpos[:2], self.data.qpos[3:5]], axis=0)
        obs_rot = np.array([self.data.qpos[2], self.data.qpos[5]])
        ...
        return obs_trasl, obs_rot, reward, done, truncated, {}, rimbalzato
    ...
    def reset(self, seed=None, options=None):
        ...
        self.data.qpos[2] = np.random.uniform(low=-3.14, high=3.14)  # Angolo casuale dell'agente
        self.data.qpos[5] = np.random.uniform(low=-3.14, high=3.14)  # Angolo casuale del target
