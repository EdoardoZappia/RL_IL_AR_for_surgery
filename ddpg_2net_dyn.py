...
RUN_DIR = f"runs_rototransl_dyn_2D/ddpg_2actor_1critic{now}"
...
class DDPGAgent(nn.Module):
    ...
    #     # --- Split state inputs for actor subnets ---
    #     states_pos = torch.cat([states[:, :2], states[:, 3:5]], dim=1)
    #     states_rot = torch.cat([states[:, 2:3], states[:, 5:6]], dim=1)
    #     next_states_pos = torch.cat([next_states[:, :2], next_states[:, 3:5]], dim=1)
    #     next_states_rot = torch.cat([next_states[:, 2:3], next_states[:, 5:6]], dim=1)
    ...
    #     with torch.no_grad():
    #         next_action_xy = torch.tanh(self.actor_target_transl(next_states_pos)) * 5.0
    #         next_action_rot = torch.tanh(self.actor_target_rot(next_states_rot))
    #         next_actions = torch.cat([next_action_xy, next_action_rot], dim=1)
    #         target_Q = self.critic_target(next_states, next_actions)
    #         y = rewards + gamma * target_Q * (1 - dones)
    ...
    #     # --- Actor updates with separate losses ---

    #     # Predicted actions
    #     pred_action_xy = torch.tanh(self.actor_transl(states_pos)) * 5.0
    #     pred_action_rot = torch.tanh(self.actor_rot(states_rot))

    #     # Detach other branch to isolate gradient
    #     pred_action_xy_det = pred_action_xy.detach()
    #     pred_action_rot_det = pred_action_rot.detach()

    #     # Loss for actor_transl (fix rotation)
    #     actor_loss_xy = -self.critic(states, torch.cat([pred_action_xy, pred_action_rot_det], dim=1)).mean()
    #     self.optimizer_actor_transl.zero_grad()
    #     actor_loss_xy.backward()
    #     self.optimizer_actor_transl.step()

    #     # Loss for actor_rot (fix translation)
    #     actor_loss_rot = -self.critic(states, torch.cat([pred_action_xy_det, pred_action_rot], dim=1)).mean()
    #     self.optimizer_actor_rot.zero_grad()
    #     actor_loss_rot.backward()
    #     self.optimizer_actor_rot.step()
