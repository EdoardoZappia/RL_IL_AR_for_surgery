...
RUN_DIR = f"TEST_NOISE/Traslazioni-dinamiche/ddpg_mov_0.05_std_0.01_{now}"
...
def train_ddpg(env=None, num_episodes=20001):
    ...
    for episode in range(num_episodes):
        ...
        state = state.clone()
        state[2:4] += torch.normal(mean=0.0, std=0.01, size=(2,), device=device)
        ...
        while not done:
            ...
            next_state[2:4] += torch.normal(mean=0.0, std=0.01, size=(2,), device=device)
