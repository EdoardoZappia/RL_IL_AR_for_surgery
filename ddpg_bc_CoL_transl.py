...
RUN_DIR = f"Esperimento_1_corretto/CoL/Traslazioni-dinamiche/Combinazioni_test/ddpg_mov_0.05_std_0.003_buffer_pieno_0.003_no-init_75_25_{now}"
...
class DDPGAgent(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.agent_buffer = ReplayBuffer(50000)
        self.batch_size = 128
        self.noise_std = 0.5
        self.min_noise_std = 0.01
        self.noise_decay = 0.999

        self.expert_buffer = ReplayBuffer(50000)
    ...
    def update(self, gamma=GAMMA, tau=TAU, update_actor=False):
        ...
        if update_actor:
            # Calcolo BC loss solo se ci sono dati esperti
            if use_only_expert:
            ...
            current_actions_exp = self.actor(expert_states)
            bc_loss = F.mse_loss(current_actions_exp, expert_actions)

            # Lambda adattivo calcolato sempre sul batch corrente
            with torch.no_grad():
            ...
            # Policy gradient su tutto il batch
...
def train_ddpg(env=None, num_episodes=10001):
    ...
    if os.path.exists(dataset_path):
        ...
        for transition in transitions:
            ...
            agent.expert_buffer.push((state, action, reward, next_state, done))
        print(f"Buffer inizializzato con {len(agent.expert_buffer)} transizioni")
    else:
    ...
    for episode in range(num_episodes):
        ...
        state = state.clone()
        state[2:4] += torch.normal(mean=0.0, std=0.003, size=(2,), device=device)
        ...
        while not done:
            ...
            next_state[2:4] += torch.normal(mean=0.0, std=0.003, size=(2,), device=device)
